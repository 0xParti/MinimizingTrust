...
# This concatenates all chapters and compiles with pandoc

cd "$(dirname "$0")"

# Create header with frontmatter
cat > /tmp/zkBook_combined.md << 'HEADER'
---
titlepage: true
...
do
    echo "" >> /tmp/zkBook_combined.md
    # Convert ```mermaid to ```{.mermaid width=400} for proper PDF sizing
    sed 's/^```mermaid$/```{.mermaid width=400}/' "$chapter" >> /tmp/zkBook_combined.md
    echo "" >> /tmp/zkBook_combined.md
done

# Compile PDF with mermaid filter
